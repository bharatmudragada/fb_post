...
class ReactionsDataType(object):
    def __init__(self, count=None, type=None,  **kwargs):
        self.count = count
        self.type = type
...
class ReactionsDataSerializer(serializers.Serializer):
    count = serializers.IntegerField(required=False, allow_null=True)
    type = serializers.ListField(child=serializers.CharField(), required=False)

    def create(self, validated_data):
        return ReactionsDataType(**validated_data)
